...
with open("serie_air2m.txt", "r") as ins:
	for line in ins:
		line=line.replace("\n","")
		temperatura.append(line.split(" ")[-1])

with open("serie_prate.txt", "r") as ins:
	for line in ins:
		line=line.replace("\n","")
		chuva.append(line.split(" ")[-1])


with open("serie_soilw.0-10cm.txt", "r") as ins:
	for line in ins:
		line=line.replace("\n","")
		umidade10cm.append(line.split(" ")[-1])


with open("serie_soilw.10-200cm.txt", "r") as ins:
	for line in ins:
		line=line.replace("\n","")
		umidade2m.append(line.split(" ")[-1])

with open("serie_fogo.txt", "r") as ins:
	for line in ins:
		line=line.replace("\n","")
		fogo.append(line.split(" ")[-1])
...
for i in range(1,len(datas)):
	#print(datas[i]+" "+temperatura[i]+" "+chuva[i]+" "+umidade10cm[i]+" "+umidade2m[i]+" "+fogo[i])
	matrix.append([datas[i],float(temperatura[i]),float(chuva[i]),float(umidade10cm[i]),float(umidade2m[i]),float(fogo[i])])
	lst_1.append(datas[i])
	lst_2.append(float(temperatura[i]))
	lst_3.append(float(chuva[i]))
	lst_4.append(float(umidade10cm[i]))
	lst_5.append(float(umidade2m[i]))
	lst_6.append(float(fogo[i]))

matrix2.append(lst_1)
matrix2.append(lst_2)
matrix2.append(lst_3)
matrix2.append(lst_4)
matrix2.append(lst_5)
matrix2.append(lst_6)
...
import numpy as np
def calc_attractor_dimension(serie,dim_min=1,dim_max=30):
	"""
...
def testeFogo():
	...
	print("teste")
	#s=matrix2[1]#temperatura
	#s=matrix2[2]#chuva

	#s=matrix2[3]#umidade10cm
	#
	s=matrix2[4]#umidade2m
	#s=matrix2[5]#fogo
	serie2=[]
	for i in range(0,len(s),4):
		serie2.append((s[i]+s[i+1]+s[i+2]+s[i+3])/4)

	serie3=[]
	for i in range(0,len(serie2),31):
		serie3.append(np.sum(serie2[i:i+31])/31)

	serie=s

	print(len(serie3),len(serie2))
	tau,cf_list=find_lag(serie)
	print("tau:",tau)
	plot1d(cf_list,"função de correlação")
	d=None
	d=calc_attractor_dimension(serie)

	series=create_lagged_series(serie,3,729)
	...
	cmap = cm.jet
	c = range(0, len(series[0]), 1)
	plt.show()
	plt.scatter(series[0],series[1], c=c, cmap=cmap)
	#teste=series[2]
	plt.show()
	plt.plot(series[0],series[1], alpha=.99)
	plt.show()
	#plt.hist(serie, bins=365)
	#plt.show()
...
def imprimeDados():
	print("Dia TEMP PREC UM10 UM2M FOGO")
	#s=matrix2[1]#temperatura
	#s=matrix2[2]#chuva
	...
	for i in range(0,len(s),4):
		serie2.append((s[i]+s[i+1]+s[i+2]+s[i+3]))
	CSV_diario.append(serie2)

	for i in range(len(CSV_diario[0])):
		s=CSV_diario
		print (s[0][i],s[1][i],s[2][i],s[3][i],s[4][i],s[5][i])
'''
	serie3=[]
	for i in range(0,len(serie2),31):
		serie3.append(np.sum(serie2[i:i+31])/31)
...
def imprimeMENSAL():
	print("MES TEMP PREC UM10 UM2M FOGO")
	#s=matrix2[1]#temperatura
	#s=matrix2[2]#chuva
	...
	s=matrix2[0]
	serie2=[]
	for i in range(0,len(s),4*30):
		serie2.append(s[i][0:6])
	CSV_diario.append(serie2)
	...
	for i in range(1,6):
		...
		for i in range(0,len(s),4):
			serie2.append((s[i]+s[i+1]+s[i+2]+s[i+3])/4)
		serie3=[]
		for i in range(0,len(serie2),30):
			serie3.append(np.sum(serie2[i:i+30])/30)
		CSV_diario.append(serie3)
